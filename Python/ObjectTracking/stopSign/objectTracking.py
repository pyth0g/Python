...
class ImgFind:
    def __init__(self):
        pass
    def StandardSearch(img,find,output,min_threshold):
    ...
    def ReturnArray(img,find,min_threshold):
        main_image = img
        if not main_image.any():
            raise FileNotFoundError(f"Cannot find file '{img}'")
        template = cv2.imread(find)
...
class ArrayFind:
    def __init__(self):
        pass
    def StandardSearch(array,find,output,min_threshold):
        if os.path.isfile(output):
            raise FileExistsError(f"File '{output}' already exists")
        main_image = array
        template = cv2.imread(find)
    ...
    def ReturnArray(array,find,min_threshold):
        main_image = array
        template = cv2.imread(find)
...
class VideoFind:
    def __init__(self):
        pass
    def StandardSearch(video,find,output,min_threshold):
        if os.path.isfile(output):
            raise FileExistsError(f"File '{output}' already exists")
        ...
        while True:
            ...
            processed_frame = ImgFind.ReturnArray(frame, find, min_threshold)
...
class AdvancedFind:
    ...
    def MostConfident(img,find,output,min_threshold,intervals=0.5):
        if os.path.isfile(output):
            raise FileExistsError(f"File '{output}' already exists")
        main_image = cv2.imread(img)
        try:
    ...
    def MostConfident_ReturnArray(img,find,min_threshold,intervals=0.5):
        main_image = cv2.imread(img)
        try:
            if not main_image:
                raise FileNotFoundError(f"Cannot find file '{img}'")
